...
| When to ask | `apps/mobile/src/fx/rates-store.ts`, `rates-provider.tsx` | The cache is read first. A request only when a screen needs a month in a currency it holds that differs from the target; one request per calendar month (the month and the week before it, through today) for the quotes it still lacks; a month asked after it ended is final; the running month again at most every 6 h; a failure is remembered for 60 s and then retried by the store itself (a screen left open recovers when the connection returns); a request in flight is tracked per (quote, month), so another currency of the same month is never blocked by it. A ledger in one currency shown in that currency never asks anything; `single` mode never asks anything. |
