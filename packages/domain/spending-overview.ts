...
export function spendingOverview(snapshot: LedgerSnapshot, period: ReportPeriod): SpendingOverview {
  if (!validDateISO(period.startISO) || !validDateISO(period.endISO) || period.startISO > period.endISO
    || !['ARS', 'USD'].includes(period.currency)) throw new Error('Período inválido.');
  ...
  const accountIds = new Set(snapshot.accounts.filter(a => a.currency === period.currency).map(a => a.id));
