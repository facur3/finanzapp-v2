...
export default function AccountsScreen() {
  ...
  const visibleAccounts = snapshot.accounts.filter(account => !hidden.has(account.id));
  const sections = availableCurrencies(visibleAccounts).map(currency => ({ currency, data: visibleAccounts.filter(account => account.currency === currency) }));
  return <>
