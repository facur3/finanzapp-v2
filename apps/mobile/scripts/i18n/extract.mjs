...
/** Generated data modules (the CLDR currency and territory names, one file per language) carry
 * this header and are data, not copy: exempt as a directory, so a new language needs no allow entry. */
export const GENERATED_DIR = 'src/i18n/currencies/';
export const GENERATED_HEADER = '/* GENERATED by apps/mobile/scripts/currency/generate.mjs';
export const isGeneratedModule = (file, source) => file.startsWith(GENERATED_DIR) && source.startsWith(GENERATED_HEADER);
...
export function findLiterals() {
  ...
  for (const file of sourceFiles('app', 'src')) {
    ...
    if (isGeneratedModule(file, source)) continue;
    source.split('\n').forEach((line, index) => {
