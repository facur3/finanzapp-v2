...
const GENERATED_HEADER = '/* GENERATED by apps/mobile/scripts/currency/generate.mjs';

/** Every catalogue language needs its generated currency-names module (src/i18n/currencies/<language>.ts,
 * written by `npm run currency:generate`), and every module there must be generated, never hand-written:
 * a language added to the catalogues without regenerating the catalogue fails here, before the build. */
export function checkGeneratedModules(languages = catalogueLanguages(), dir = join(MOBILE, 'src', 'i18n', 'currencies')) {
  const errors = [];
  for (const language of languages) {
    const file = join(dir, language + '.ts');
    if (!existsSync(file)) { errors.push(`${language}: src/i18n/currencies/${language}.ts is missing: run npm run currency:generate`); continue; }
    if (!readFileSync(file, 'utf8').startsWith(GENERATED_HEADER)) errors.push(`${language}: src/i18n/currencies/${language}.ts is not the generator's output`);
  }
  for (const name of existsSync(dir) ? readdirSync(dir) : []) {
    if (!/^[a-z]{2,3}\.ts$/.test(name)) continue;
    const language = name.slice(0, -3);
    if (!languages.includes(language)) errors.push(`src/i18n/currencies/${name} names a language without a catalogue (${language})`);
  }
  return errors;
}
...
export async function checkCatalogues({ strict = false } = {}) {
  ...
  const report = { errors: checkGeneratedModules(), stale: [], notes: [] };
