...
export function DebtForm({ original }: { original?: PersonalDebtProfile }) {
  ...
  function profileBase() {
    return {
      ...
      counterparty: before?.counterparty ?? counterparty.trim(),
  ...
  async function save() {
    ...
    try {
      if (before) {
        ...
        if (!submission) {
          const candidate: PersonalDebtProfile = { ...profileBase(), active: before.active,
  ...
  function archive() {
    ...
    Alert.alert('¿Archivar esta deuda?', 'Los pagos o cobros anteriores siguen guardados. Solo deja de aparecer como obligación activa.', [
  ...
  return <Screen>
    ...
    {before && account ? <Surface grouped>
      <DetailRow label={before.direction === 'owed_by_me' ? 'Debo a' : 'Me debe'} value={before.counterparty} />
      ...
      <Field label={direction === 'owed_by_me' ? 'Persona o entidad' : 'Persona o cliente'}
        value={counterparty} onChangeText={setCounterparty} placeholder="Nombre o concepto"
        maxLength={80} autoCapitalize="words" editable={!locked} />
      <Choices value={currency} onChange={setCurrency} disabled={locked}
        options={[{ value: 'ARS', label: 'Pesos · ARS' }, { value: 'USD', label: 'Dólares · USD' }]} />
      <AmountField label={direction === 'owed_by_me' ? 'Monto que debés' : 'Monto que te deben'}
        currency={currency} value={amount} onChangeText={value => { setAmount(value); setError(null); }} editable={!locked} />
    </>}

    <View style={{ gap: 8 }}>
      <AppText secondary style={{ fontSize: 14, fontWeight: '500' }}>Vencimiento</AppText>
    ...
    <Field label="Nota (opcional)" value={note} onChangeText={setNote} maxLength={120}
      autoCapitalize="sentences" editable={!locked} />

    <AppText secondary style={{ fontSize: 13 }}>
      Los pagos y cobros reducen el saldo mediante transferencias. No se crean ingresos o gastos falsos al saldar una deuda.
    </AppText>
    ...
    <ActionButton label={error && (pendingCreate || pendingEdit) ? 'Reintentar guardado' : before ? 'Guardar cambios' : 'Crear deuda'}
      onPress={save} busy={busy} disabled={before ? false : !counterparty.trim() || !amount.trim()} />
    {before && <ActionButton label={pendingArchive && error ? 'Reintentar archivo' : 'Archivar deuda'}
