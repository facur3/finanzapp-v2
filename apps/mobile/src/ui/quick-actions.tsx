...
/** The material of a round action: an opaque, native-feeling surface rather
 * than a blurred panel. Light: white with a hairline edge and the soft card
 * shadow. Dark: a surface step with a faint light edge on top, the way a
 * control catches light. The Assistant is the same object in the brand tint:
 * a cobalt wash and a thin cobalt ring, so it reads as the first of a family,
 * not as a different kind of button. Real blur (Liquid Glass) stays for the
 * development-build phase; Expo Go gets no decorative dependency. */
 ...
 * detail keeps the three movements. */
export function QuickActions({ currency, accountId, assistant = false }: { currency?: Currency; accountId?: string; assistant?: boolean }) {
  ...
  const actions: Action[] = [
    ...(assistant ? [{ kind: 'assistant' as const, label: 'Asistente', accessibilityLabel: 'Abrir el Asistente', icon: 'sparkles' as const, tone: 'primary' as const,
      onPress: () => router.push({ pathname: '/assistant', params: currency ? { currency } : {} }) }] : []),
...
function QuickAction({ label, accessibilityLabel, icon, tone, onPress }: Action) {
  ...
  return <PressFeedback accessibilityRole="button" accessibilityLabel={accessibilityLabel} onPress={onPress} containerStyle={{ flex: 1, minWidth: 0 }}
    ...
    <View style={[{ width: QUICK_ACTION_SIZE, height: QUICK_ACTION_SIZE, borderRadius: QUICK_ACTION_SIZE / 2, alignItems: 'center', justifyContent: 'center' }, quickActionMaterial(p, assistant)]}>
