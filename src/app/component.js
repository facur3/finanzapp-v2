class Component extends DCLogic {
  ...
  moveBackSwipe(event){const gesture=this._backSwipe,touch=event&&event.touches&&event.touches[0];if(!gesture||!touch)return;const dx=Math.max(0,touch.clientX-gesture.x),dy=Math.abs(touch.clientY-gesture.y),deadZone=18;if(!gesture.tracking&&dy>dx&&dy>10){this._backSwipe=null;return;}if(dx<deadZone)return;gesture.tracking=true;gesture.lastX=touch.clientX;gesture.lastAt=Date.now();if(event.cancelable)event.preventDefault();const width=(typeof window!=='undefined'&&window.innerWidth)||390,visualDx=Math.max(0,dx-deadZone);this.setState({swipeState:'tracking',swipeOffset:Math.min(visualDx,width*.92)});}
