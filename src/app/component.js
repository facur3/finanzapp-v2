class Component extends DCLogic {
  ...
  completeBackSwipe(){if(this.state.confirm){this.setState({confirm:null,swipeState:'idle',swipeOffset:0});return;}if(this.state.sheet==='assistant')this.stopAssistantListening();const previous=this.previousNavigation();if(!previous){this.setState({swipeState:'idle',swipeOffset:0});return;}this._restoringNavigation=true;this.setState({...previous,swipeState:'idle',swipeOffset:0,navState:'back-settle'});}
  ...
  popScreen(target=null,patch={}){
    ...
    const previous=this.previousNavigation(target);
  ...
  confirmAssistantDraft(){const d=this.hydrateAssistantDraft(this.state.assistantDraft);const missing=this.assistantMissing(d);if(missing.length){this.setState({assistantError:'Falta '+missing.join(', ')+'. Tocá Editar para completarlo sin repetir la frase.'});return;}const dateLabel=window.FinanzDomain.labelFromISO(d.dateISO);
    ...
    if(d.intent==='card_payment'){const idx=this.state.cards.findIndex(c=>c.id===d.cardId),card=this.state.cards[idx];const amount=d.amount||this.cardResumen(card);const payDate=new Date(d.dateISO+'T12:00:00');const full=amount>=Math.round(this.cardResumen(card))-1&&this.cardResumen(card)>0;const cycle=this._dueCycle(card,payDate);const accountCurrency=(this.state.accounts[d.accountId]||{}).currency||'ARS';if(accountCurrency==='USD'&&!this.state.usdRate){this.setState({assistantError:'Necesito una cotización actualizada para pagar una tarjeta en pesos desde una cuenta en dólares.'});this.fetchPrices(true);return;}const accountDebit=window.FinanzDomain.convertCurrency(amount,'ARS',accountCurrency,this.state.usdRate);
      ...
      if((this.state.accounts[d.accountId]||{}).balanceKnown!==false&&(Number(this.state.balances[d.accountId])||0)+0.005<accountDebit){this.setState({assistantError:'La cuenta elegida no tiene saldo suficiente para registrar ese pago.'});return;}
      this.setState(s=>{const cards=s.cards.map((c,i)=>i===idx?this._applyCardPayment({...c,pagos:[{name:'Pago '+c.brand,monto:amount,date:dateLabel,dateISO:d.dateISO,txnId:s._next},...(c.pagos||[])]},amount,payDate):c);const txn={id:s._next,type:'pago',merchant:'Pago '+card.brand,cat:'pago',amount:-amount,val:amount,currency:'ARS',account:d.accountId,accountAmount:accountDebit,card:card.id,cardRollback:this.snapshotCard(card),isTransfer:true,dateLabel,dateISO:d.dateISO,note:'Pago de tarjeta · Asistente',tags:['asistente']};return{cards,balances:{...s.balances,[d.accountId]:(s.balances[d.accountId]||0)-accountDebit},txns:[txn,...s.txns],_next:s._next+1,sheet:null,assistantDraft:null,assistantText:'',flash:full?'Resumen pagado':'Pago registrado'};});return;}
  ...
  coercePersistedState(st){
    ...
    out.cards=(Array.isArray(out.cards)?out.cards:[]).filter(c=>c&&typeof c==='object').map((c,index)=>{
      ...
      return{...record(c),id,brand:String(c.brand||'Visa').slice(0,20),bank:String(c.bank||'').slice(0,50),last4:String(c.last4||'0000').replace(/\D/g,'').slice(-4).padStart(4,'0'),saldo,statementPaid:Math.min(saldo,Math.max(0,finite(c.statementPaid))),statementAdjustment:finite(c.statementAdjustment),pendingAdjustment:finite(c.pendingAdjustment),limit:Math.max(0,finite(c.limit)),cierre:String(c.cierre||'—').slice(0,20),vence:String(c.vence||'—').slice(0,20),cierreDay:dayFrom(c.cierre,'cierreDay'),venceDay:dayFrom(c.vence,'venceDay'),grad:String(c.grad||this.CARDGRADS[0]),autopay:!!c.autopay,autopayAccount:out.accounts[c.autopayAccount]?c.autopayAccount:'',compras:moneyRows(c.compras),cuotas:moneyRows(c.cuotas),pendientes:moneyRows(c.pendientes).map(row=>({...row,installments:Math.max(1,Math.round(finite(row.installments,1)))})),pagos:moneyRows(c.pagos)};
    });
  ...
  deleteAccount(k){if(this.state.txns.some(t=>[t.account,t.from,t.to,t.cashAccount,t.portfolio].indexOf(k)>=0)||this.state.recurring.some(r=>r.targetKind==='account'&&r.targetId===k)||this.state.cards.some(c=>c.autopayAccount===k)){this.flashMsg('Archivá la cuenta para conservar movimientos y automatizaciones');return;}this.setState(s=>{const order=s.order.filter(x=>x!==k),accounts={...s.accounts},balances={...s.balances},assets={...s.assets},accMeta={...s.accMeta},archived={...s.archived};delete accounts[k];delete balances[k];delete assets[k];delete accMeta[k];delete archived[k];return{order,accounts,balances,assets,accMeta,archived,push:null,tab:'cuentas',flash:'Cuenta eliminada'};});}
  ...
  runAutomations(){const today=new Date();today.setHours(0,0,0,0);
    this.setState(s=>{let txns=s.txns,balances={...s.balances},cards=s.cards.map(c=>({...c})),next=s._next,categoryTotals={...s.categoryTotals},monthIncome=s.monthIncome,monthExpense=s.monthExpense,changed=false,posted=0;
      ...
      cards=cards.map(c=>{if(!c.autopay||!c.autopayAccount||s.archived[c.autopayAccount])return c;const due=this._parseDate(c.vence,today);const cyc=this._cycleKey(due||today);if(c.paidCycle===cyc)return c;if(!due||today<due)return c;const resumen=this.cardResumen(c);if(resumen<=0){const gross=window.FinanzDomain.cardStatementGross(c);return gross>0||Number(c.statementPaid)>0?this._applyCardPayment(c,0,today):this._closeCycleCard(c,today);}const rollback=this.snapshotCard(c),accountCurrency=(s.accounts[c.autopayAccount]||{}).currency||'ARS';if(accountCurrency==='USD'&&!s.usdRate)return c;const accountDebit=window.FinanzDomain.convertCurrency(resumen,'ARS',accountCurrency,s.usdRate);if((balances[c.autopayAccount]||0)<accountDebit)return c;const paymentISO=window.FinanzDomain.todayKey(today);balances={...balances,[c.autopayAccount]:(balances[c.autopayAccount]||0)-accountDebit};txns=[{id:next,type:'pago',merchant:'Pago '+c.brand,cat:'pago',amount:-resumen,val:resumen,currency:'ARS',account:c.autopayAccount,accountAmount:accountDebit,card:c.id,cardRollback:rollback,isTransfer:true,dateLabel:this._monthLabel(today),dateISO:paymentISO,note:'Pago automático',tags:[]},...txns];const paymentId=next;next++;posted++;changed=true;return this._applyCardPayment({...c,pagos:[{name:'Pago automático',monto:resumen,date:this._monthLabel(today),dateISO:paymentISO,txnId:paymentId},...(c.pagos||[])]},resumen,today);});
  ...
  openAssetTrade(mode,account,type='CEDEAR'){
    ...
    if(existing){this.setState({push:'assetTrade',sheet:null,subsheet:null,atSub:null,atMode:mode||'buy',atAccount:existing,atSource:source,atType:type,atTicker:'',atName:'',atEmoji:'',atSearch:'',atQty:'',atTotal:'',atDateISO:this._todayKey()});return;}
  ...
  tradeAsset(mode,account,a){const isC=(this.CRYPTOS||[]).some(x=>x[0]===a.ticker);const isB=(this.BONOS||[]).some(x=>x[0]===a.ticker)||a.unitDivisor===100;const atType=a.fci?'FCI':isC?'Cripto':isB?'Bono/ON':'CEDEAR';this.setState({push:'assetTrade',sheet:null,subsheet:null,atSub:null,atMode:mode,atAccount:account,atSource:this.liquidIds()[0]||'',atType,atTicker:a.ticker,atName:a.name,atEmoji:a.emoji,atSearch:'',atQty:'',atTotal:'',atDateISO:this._todayKey()});}
  ...
  renderVals(){
    ...
    else if(assistantDraft&&assistantMissing.some(label=>label==='la cuenta'||label==='la cuenta de pago'||label==='la cuenta o tarjeta'||/^una cuenta en /.test(label))){const wantedCurrency=assistantDraft.currencyExplicit?assistantDraft.currency:null;assistantChoiceTitle=assistantDraft.intent==='card_payment'?'Elegí desde qué cuenta pagar':'Elegí cómo se pagó';const accountIds=assistantDraft.transactionType==='gasto'?[...LIQ,...FCI_SPEND.map(source=>source.id)]:LIQ;assistantChoiceOptions=accountIds.map(id=>this.spendSourceMeta(id,S)).filter(meta=>meta&&(!wantedCurrency||meta.currency===wantedCurrency)).map(meta=>({label:meta.name,emoji:meta.emoji||'🏦',onPick:()=>this.setState({assistantDraft:{...assistantDraft,accountId:meta.id,cardId:''},assistantError:''})}));if(assistantDraft.intent==='transaction'&&assistantDraft.transactionType==='gasto'&&wantedCurrency!=='USD')assistantChoiceOptions.push(...S.cards.map(card=>({label:card.brand+' ·••• '+card.last4,emoji:'💳',onPick:()=>this.setState({assistantDraft:{...assistantDraft,accountId:'',cardId:card.id,cardExplicit:true,currency:'ARS'},assistantError:''})})));}
    ...
    if(S.assetView){const av=S.assetView;const a=(S.assets[av.account]||[]).find(x=>x.ticker===av.ticker);
      if(a){const lp=a.lastPrice||a.avg,value=FD.assetValueARS(a,S.usdRate),performanceValue=FD.assetPerformanceValueARS(a,S.usdRate),cost=FD.assetCostARS(a,S.usdRate),gl=performanceValue-cost,glPct=cost>0?gl/cost*100:0;const accM=ACC[av.account]||{};const isCrypto=this.CRYPTOS.some(x=>x[0]===a.ticker),isBond=this.BONOS.some(x=>x[0]===a.ticker)||a.unitDivisor===100;const unitKind=a.fci?'cuotaparte':isCrypto?'unidad':isBond?'100 nominales':'CEDEAR';
        ...
        assetD={adAName:a.name,adATicker:a.ticker||'',adAEmoji:a.emoji,adAFillVar:accM.fillVar||'--cat-inversion-fill',
          ...
          adABack:()=>this.popScreen(null,{assetView:null})};
    ...
    {const i=S.cardView,c=S.cards[i]||S.cards[0]||{limit:0,brand:'',bank:'',last4:'',grad:this.CARDGRADS[0],cierre:'—',vence:'—',compras:[],cuotas:[],pendientes:[],pagos:[]};const saldo=cardSaldo(i),pendingTotal=FD.cardPendingTotal(c);const hasLimit=Number(c.limit)>0,avail=hasLimit?Math.max(0,c.limit-saldo):0;
      cardD={cdBrand:c.brand,cdBank:c.bank,cdLast4:c.last4,cdGrad:c.grad,cdSaldoStr:money(saldo),cdResumenStr:money(cardResumen(c)),cdPendingTotalStr:money(pendingTotal),cdHasPending:pendingTotal>0.005,cdDeudaStr:money(saldo),cdHasLimit:hasLimit,cdLimitStr:hasLimit?moneyInt(c.limit):'No cargado',cdAvailStr:moneyInt(avail),cdAvailPct:hasLimit?Math.max(0,Math.min(100,Math.round(avail/c.limit*100)))+'%':'0%',cdCierre:c.cierre,cdVence:c.vence,cdHasPreviousCycle:!!(c.previousClose&&c.previousDue),cdPreviousCycleStr:c.previousClose&&c.previousDue?('Ciclo anterior · cerró '+c.previousClose+' · venció '+c.previousDue):'',
        ...
        cdPay:()=>this.setState({push:'payCard',payAmount:'',payAccount:LIQ[0]||'banco'}),cdAddPurchase:()=>this.openCardPurchase(i),cdEdit:()=>this.openAddCard(i),cdDelete:()=>this.requestConfirm({title:'Eliminar tarjeta',msg:'Se eliminará esta tarjeta. No se puede deshacer.',confirmLabel:'Eliminar',danger:true,onConfirm:()=>this.deleteCard(i)}),
    ...
    const masItems=[
      ...
      {label:'Inversiones',sub:'CEDEARs, FCI, cripto, bonos y ON',emoji:'📈',fillVar:'--cat-inversion-fill',onPick:()=>this.setState({push:'investments'})},
