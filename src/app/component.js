class Component extends DCLogic {
  ...
  componentDidMount(){
    ...
    this._automationWake=()=>{if(document.visibilityState!=='hidden')this.runAutomations();};
  ...
  applySetupImport(imported){const setup=imported.setup;
    this.setState(s=>{const accounts={...s.accounts},balances={...s.balances},assets={...s.assets},order=[...s.order];let cards=[...(s.cards||[])],recurring=[...(s.recurring||[])],txns=[...(s.txns||[])],next=s._next;
      ...
      setup.accounts.forEach((account,index)=>{let id=account.id;const same=accounts[id]&&accounts[id].kind===account.kind;if(accounts[id]&&!same){const byName=Object.keys(accounts).find(key=>accounts[key].kind===account.kind&&String(accounts[key].name).toLowerCase()===account.name.toLowerCase());id=byName||account.id+'_importado';let suffix=2;while(accounts[id]&&id!==byName){id=account.id+'_importado_'+suffix;suffix++;}}
        const pal=this.PALETTE[(order.length+index)%this.PALETTE.length];accounts[id]={...(accounts[id]||{}),name:account.name,type:account.type,kind:account.kind,currency:account.currency,liquid:account.liquid,emoji:account.emoji,iconVar:(accounts[id]||{}).iconVar||pal[0],fillVar:(accounts[id]||{}).fillVar||pal[1]};balances[id]=account.balance;if(order.indexOf(id)<0)order.push(id);if(!assets[id])assets[id]=[];accountIds.set(account.id,id);});
      ...
      Object.keys(assets).forEach(id=>{if(accounts[id]&&accounts[id].kind==='invest')balances[id]=(assets[id]||[]).reduce((sum,asset)=>sum+Number(asset.qty||0)*Number(asset.lastPrice||asset.avg||0),0);});
      ...
      const existingLots=new Set(txns.map(t=>t.importKey).filter(Boolean));setup.investmentLots.forEach(lot=>{if(existingLots.has(lot.importKey))return;const accountId=accountIds.get(lot.accountId)||lot.accountId;txns.push({id:next,type:'inversion',ticker:lot.ticker,aqty:lot.qty,merchant:'Compra '+lot.ticker,cat:'inversion',amount:-lot.total,val:lot.total,currency:'ARS',from:accountId,to:accountId,isTransfer:true,dateLabel:window.FinanzDomain.labelFromISO(lot.dateISO),dateISO:lot.dateISO,note:lot.note||((lot.qty<1?lot.qty:this.fmtInt(lot.qty))+' '+lot.ticker+' · importado'),tags:['importado'],importKey:lot.importKey});existingLots.add(lot.importKey);next++;});
  ...
  async fetchPrices(silent=false){if(this.state.pricesLoading)return;this.setState({pricesLoading:true});
    try{const s=this.state;const na={...s.assets};const nb={...s.balances};
      ...
      // 4. Apply every asset in every account. An investment account may mix an
      // FCI with CEDEARs/bonds; finding a fund must never short-circuit the rest.
      ...
      Object.keys(na).forEach(k=>{if(!s.accounts[k])return;const aa=[...(na[k]||[])];if(!aa.length)return;
        let changed=false;
        aa.forEach((a,i)=>{if(a.fci){const live=fciPrices[a.id]||0;if(live<=0)return;const lastVcp=Number(a.lastVcp)||0;const previousValue=Number(a.qty||0)*Number(a.lastPrice||a.avg||0);let units=Number(a.units)||0;
            if(!units&&a.unitsEstimated)units=previousValue/live;
            else if(!units&&Number(a.qty)>0)units=Number(a.qty);
            else if(!units&&aa.length===1)units=(Number(s.balances[k])||0)/live;
            if(aa.length===1&&units>0&&lastVcp>0&&!a.unitsEstimated){const expected=units*lastVcp,cur=Number(s.balances[k])||0;if(Math.abs(cur-expected)>0.5)units=cur/lastVcp;}
            const dayKey=new Date().toISOString().slice(0,10);let dayVcp=Number(a.dayVcp)||0;let dKey=a.dayKey;if(dKey!==dayKey||dayVcp<=0){dayVcp=lastVcp>0?lastVcp:live;dKey=dayKey;}
            const fundValue=units*live,rPct=dayVcp>0?(live-dayVcp)/dayVcp:0,rAmt=Math.round(fundValue*(live-dayVcp)/(live||1));newAccMeta[k]={chg:(rPct>=0?'+':'')+(rPct*100).toFixed(2).replace('.',',')+'%',up:rPct>=0,rend:rAmt};
            aa[i]={...a,qty:units,units,lastPrice:live,lastVcp:live,dayVcp,dayKey:dKey};changed=true;return;}
          if(!a.ticker)return;let np=0;
        ...
        const total=aa.reduce((s2,a)=>s2+Number(a.qty||0)*Number(a.lastPrice||a.avg||0),0);
  ...
  renderVals(){
    ...
    if(S.investView&&ACC[S.investView]){const k=S.investView,a=ACC[k];
      ...
      const assets=rawAssets.map(as=>{const lp=as.lastPrice||as.avg;const value=as.qty*lp;const cost=as.qty*as.avg;const gl=value-cost;const glPct=cost>0?(gl/cost*100):0;const unknown=!!as.costUnknown;return{name:as.name,ticker:as.ticker,emoji:as.emoji,qtyStr:(as.qty<1?as.qty.toFixed(6).replace(/\.?0+$/,''):this.fmtInt(as.qty))+(as.ticker?' '+as.ticker:' u'),avgStr:unknown?'Pendiente':money(as.avg),costStr:unknown?'Costo pendiente':money(cost),lastPriceStr:money(lp),valueStr:money(value),glStr:unknown?'Sin calcular':((gl>=0?'+':'-')+sym+this.fmtInt(Math.abs(displayARS(gl)))),glPctStr:unknown?'Cargá el costo':((gl>=0?'+':'')+glPct.toFixed(1).replace('.',',')+'%'),glColor:unknown?'var(--text-3)':gl>=0?'var(--pos)':'var(--danger)',manual:(!as.ticker&&!as.fci),onUpdatePrice:()=>this.setState({ivSub:'updatePrice',upTicker:as.ticker,atNewPrice:''})};});
      ...
      const knownAssets=rawAssets.filter(as=>!as.costUnknown);const knownValue=knownAssets.reduce((sum,as)=>sum+as.qty*(as.lastPrice||as.avg),0);
      const totalCost=knownAssets.reduce((sum,as)=>sum+as.qty*as.avg,0);
    ...
    let portAssets=[];
    INV.forEach(k=>{(S.assets[k]||[]).forEach(a=>{const lp=a.lastPrice||a.avg;const isCrypto=this.CRYPTOS.some(x=>x[0]===a.ticker);const isBond=this.BONOS.some(x=>x[0]===a.ticker);const kind=a.fci?'fci':isCrypto?'crypto':isBond?'bonds':'cedears';portAssets.push({account:k,id:a.id,ticker:a.ticker,name:a.name,emoji:a.emoji,qty:a.qty,avg:a.avg,lp,value:a.qty*lp,cost:a.qty*a.avg,costUnknown:!!a.costUnknown,kind});});});
    portAssets.sort((a,b)=>b.value-a.value);
    const portValue=portAssets.reduce((s2,a)=>s2+a.value,0);
    const knownPortAssets=portAssets.filter(a=>!a.costUnknown);const knownPortValue=knownPortAssets.reduce((s2,a)=>s2+a.value,0);
    ...
    portAssets.forEach((a,i)=>{a.color=PALCOLORS[i%PALCOLORS.length];a.pct=portValue>0?a.value/portValue*100:0;a.gl=a.costUnknown?null:a.value-a.cost;a.glPct=!a.costUnknown&&a.cost>0?a.gl/a.cost*100:0;});
    ...
    if(S.assetView){const av=S.assetView;const a=(S.assets[av.account]||[]).find(x=>x.ticker===av.ticker);
      if(a){const lp=a.lastPrice||a.avg,value=a.qty*lp,cost=a.qty*a.avg,gl=value-cost,glPct=cost>0?gl/cost*100:0;const accM=ACC[av.account]||{};const isCrypto=this.CRYPTOS.some(x=>x[0]===a.ticker),isBond=this.BONOS.some(x=>x[0]===a.ticker);const unitKind=a.fci?'fondo':isCrypto?'unidad':isBond?'bono':'CEDEAR';
