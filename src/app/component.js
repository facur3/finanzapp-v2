class Component extends DCLogic {
  ...
  savePurchase(){const S=this.state;const val=parseFloat((S.cpAmount||'').replace(',','.'))||0;if(!val){this.flashMsg('Ingresá un monto');return;}const idx=S.cpCard;const card=S.cards[idx];if(!card){this.flashMsg('Agregá una tarjeta primero');return;}if(!S.categories[S.cpCat]){this.flashMsg('Elegí una categoría');return;}const n=S.cpInstall;const merchant=S.cpMerchant.trim()||(this.state.categories[S.cpCat]?this.state.categories[S.cpCat].name:'Compra');const label=n>1?(merchant+' · cuota 1/'+n):merchant;const dateISO=window.FinanzDomain.isoFromLabel(S.cpDateISO||S.cpDate);
    this.setState(s=>{const cuota=n>1?{name:merchant,cur:1,tot:n,monto:val/n}:null;const cs=s.cards.map((c,i)=>i===idx?{...c,saldo:c.saldo+val,compras:[{name:label,monto:val,date:window.FinanzDomain.labelFromISO(dateISO),dateISO},...(c.compras||[])],cuotas:cuota?[cuota,...(c.cuotas||[])]:c.cuotas}:c);const ct={...s.categoryTotals,[S.cpCat]:(s.categoryTotals[S.cpCat]||0)+val};const t={id:s._next,type:'gasto',merchant:label,cat:S.cpCat,amount:-val,val,currency:'ARS',card:card.id,onCard:true,dateLabel:window.FinanzDomain.labelFromISO(dateISO),dateISO,note:n>1?('Tarjeta '+card.brand+' · '+n+' cuotas'):('Tarjeta '+card.brand),tags:[]};return{cards:cs,categoryTotals:ct,monthExpense:s.monthExpense+val,txns:[t,...s.txns],_next:s._next+1,push:'cardDetail',cardView:idx,flash:'Compra registrada'};});}
  ...
  saveAssetTrade(){const S=this.state;const qty=this.parseNum(S.atQty);const total=this.parseNum(S.atTotal);const buy=S.atMode==='buy';
    ...
    this.setState(s=>{const na={...s.assets};const aa=[...(na[inv]||[])];const xi=aa.findIndex(a=>a.ticker===ticker);
      ...
      const tradeISO=window.FinanzDomain.isoFromLabel(S.atDateISO||this._todayKey());const t={id:s._next,type:'inversion',ticker,aqty:qty,merchant:(buy?'Compra ':'Venta ')+ticker,cat:'inversion',amount:buy?-total:total,val:total,currency:'ARS',from:buy?inv:inv,to:inv,isTransfer:true,dateLabel:window.FinanzDomain.labelFromISO(tradeISO),dateISO:tradeISO,note:(qty<1?qty:this.fmtInt(qty))+' '+ticker+' · '+this.fmtInt(total),tags:[]};
  ...
  renderVals(){
    ...
    if(S.assetView){const av=S.assetView;const a=(S.assets[av.account]||[]).find(x=>x.ticker===av.ticker);
      if(a){const lp=a.lastPrice||a.avg,value=a.qty*lp,cost=a.qty*a.avg,gl=value-cost,glPct=cost>0?gl/cost*100:0;const accM=ACC[av.account]||{};const isCrypto=this.CRYPTOS.some(x=>x[0]===a.ticker),isBond=this.BONOS.some(x=>x[0]===a.ticker);const unitKind=isCrypto?'unidad':isBond?'bono':'CEDEAR';
        ...
        const lots=sortedTxns.filter(t=>t.type==='inversion'&&t.ticker===av.ticker).map(t=>{const buy=(t.amount||0)<=0;const q=t.aqty;return{date:FD.timelineLabelFromISO(t.dateISO||FD.isoFromLabel(t.dateLabel)),kind:buy?'Compraste':'Vendiste',qtyStr:q!=null?((q<1?String(q):this.fmtInt(q))+' '+av.ticker):'',amountStr:sym+this.fmtInt(Math.abs(t.amount||t.val||0)),color:buy?'var(--text)':'var(--pos)'};});
    ...
    return {
      ...
      heroSub:(S.balanceMode==='disponible'?'Disponible para usar ahora':'Neto · cuentas + inversiones − deudas')+(heroIsUsd?' · dólar cripto':'')+' · Tocá para ver en '+(heroIsUsd?'pesos':'dólares'),
      ...
      atQtyDisplay:this.fmtThousands(S.atQty),setAtQty:(e)=>this.setState({atQty:this.cleanNum(e.target.value)}),atDateISO:S.atDateISO,atDateMax:FD.todayKey(),setAtDate:(e)=>this.setState({atDateISO:e.target.value}),
